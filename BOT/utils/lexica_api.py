import asyncio
from lexica import AsyncClient
from .misc import ImageModels
from httpx import ReadTimeout, Timeout, AsyncClient as HTTPXAsyncClient

async def ImageGeneration(model, prompt):
    try:
        if model not in list(ImageModels.values()):
            return 1
        client = AsyncClient()
        output = await client.generate(model, prompt, "")
        if output['code'] != 1:
            return 2
        task_id, request_id = output['task_id'], output['request_id']
        await asyncio.sleep(20)
        tries = 0
        image_url = None
        resp = await client.getImages(task_id, request_id)
        while True:
            if resp['code'] == 2:
                image_url = resp['img_urls']
                break
            if tries > 15:
                break
            await asyncio.sleep(5)
            resp = await client.getImages(task_id, request_id)
            tries += 1
            continue
        return image_url
    except Exception as e:
        raise Exception(f"Failed to generate the image: {e}")

async def UpscaleImages(image: bytes) -> str:
    """
    Upscales an image and returns the upscaled image path.
    """
    try:
        # Configure the HTTPX client with a timeout
        httpx_client = HTTPXAsyncClient(timeout=Timeout(30.0))
        client = AsyncClient(httpx_client=httpx_client)

        tries = 0
        while tries < 3:  # Retry up to 3 times
            try:
                content = await client.upscale(image)
                break
            except ReadTimeout:
                tries += 1
                if tries >= 3:
                    raise Exception("Failed to upscale the image after multiple attempts due to timeout.")
                await asyncio.sleep(5)  # Wait before retrying
        await client.close()
        upscaled_file_path = "upscaled.png"
        with open(upscaled_file_path, "wb") as output_file:
            output_file.write(content)
        return upscaled_file_path
    except Exception as e:
        raise Exception(f"Failed to upscale the image: {e}")

async def gpt(prompt) -> str:
    try:
        client = AsyncClient()
        output = await client.gpt(prompt)
        await client.close()
        return output['content']
    except Exception as E:
        raise Exception(f"Failed to upscale the image: {E}")
