...
from ..helper.cache import (ADMIN_CACHE, TEMP_ADMIN_CACHE_BLOCK, admin_cache_reload)
from ..helper.status import *

from ..helper.extraction import extract_user
from ..helper.parser import mention_html
...
async def adminlist_show(_, m: Message):
    ...
    try:
        ...
        mention_users = [
            (
                f"{admin[1]} ({admin[3]})" if admin[3] else admin[1],
                "Owner" if admin[4] else ""
            )
            if admin[1].startswith("@")
            else (await mention_html(f"{admin[1]} ({admin[3]})" if admin[3] else admin[1], admin[0]),
                  "Owner" if admin[4] else "")
            for admin in user_admins
            if not admin[2]
        ]
        mention_users.sort(key=lambda x: x[0])
        ...
        mention_bots.sort(key=lambda x: x[0])

        adminstr += "<b>ADMINS:</b>\n"
        adminstr += "\n".join(f"- {i[0]} {' - ' + i[1] if i[1] else ''}" for i in mention_users)
        adminstr += "\n\n<b>BOTS:</b>\n"
        ...
        if str(ef) == str(m.chat.id):
            ...
            await m.reply_text(
                text=f"Some error occurred, <b>Error:</b> <code>{ef}</code>"
            )
...
async def promote_usr(c: app, m: Message):
    ...
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(
            text="Please specify a user or reply to a user's message to promote them."
        )
        return
    try:
    ...
    if not bot.privileges.can_promote_members:
        return await m.reply_text(
            "I lack the necessary permissions to promote members. 🔒",
        ) 
    try:
    ...
    if user_id in admin_list:
        await m.reply_text(
            "This user is already an admin. Re-promotion is not possible. ⚠️",
        )
        return
    try:
...
<b>• EVENT: PROMOTION
➖➖➖➖➖➖➖➖➖➖➖➖
👤 Promoted By: {await mention_html(m.from_user.first_name, m.from_user.id)}
...
""", reply_markup=keyboard
        ...
        await m.reply_text(text="I need admin rights to perform this action.")
    except RightForbidden:
        await m.reply_text(text="I don't have sufficient rights to promote this user.")
    except UserAdminInvalid:
        await m.reply_text(
            text="Cannot act on this user. Perhaps I wasn't the one who changed their permissions."
        )
    except RPCError as e:
        await m.reply_text(
            text=f"An error occurred: <b>Error:</b> <code>{e}</code>"
        )
