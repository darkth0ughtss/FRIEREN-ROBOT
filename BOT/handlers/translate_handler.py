...
async def translate_reply(client: Client, message: Message):
    try:
        ...
        if len(args) < 2:
            await message.reply_text("𝗨𝘀𝗮𝗴𝗲: /𝘁𝗿 '𝘁𝗮𝗿𝗴𝗲𝘁_𝗹𝗮𝗻𝗴'")
            return
        ...
        # Translate the text
        translated_text = await translator.translate(source_text, lang_tgt=target_lang)
...
async def detect_and_translate(client: Client, message: Message):
    try:
        ...
        detected_lang = await translator.detect(source_text)
        
        # Translate the text
        translated_text = await translator.translate(source_text, lang_tgt=target_lang, lang_src=detected_lang)
