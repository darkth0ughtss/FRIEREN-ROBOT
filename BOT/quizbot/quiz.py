from ..bot import bot
from ..db.users import add_user, id_to_username, add_score, get_top_users
...
async def quiz(_, message: Message):
    ...
    for i in range(number):
        ...
        # Debug print to check question and answers
        print(f"Question: {text}, Options: {answers}, Correct: {correct}")
    ...
    y = 0
    for user in users:
        await add_score(user["id"], user["points"])
        average_time = int(user["total_time"] / number)
        text += f"{get_prefix(y)} @{await id_to_username(user['id'])}: **{user['points']}** ({format_time(average_time)})\n"
        y += 1
...
async def answer(_, query):
    ...
    if user not in userlist:
        ...
        await add_user(user, query.from_user.username)
